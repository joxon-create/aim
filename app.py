...
case_names = [
    ...
    "Ocean Treasure", "Galaxy Drop", "Inferno Case", "Thunder Strike", "Ultimate 300UC"
]
...
for i, name in enumerate(case_names):
    ...
    for item in PUBG_ITEMS_POOL:
        copied = item.copy()
        if i == 19 and copied["name"] == "M416 'Lednik' (Lv.1)":
            copied["val"] = price * 30
            copied["chance"] = 0.000001
        else:
    ...
    <head>
        ...
        <style>
            * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }}
            ...
            .balance-container {{ background: #1a233a; border: 1px solid #2a3a5a; padding: 6px 14px; border-radius: 20px; font-weight: bold; color: #ffcc00; font-size: 15px; }}
            .container {{ max-width: 1200px; margin: 0 auto; width: 100%; padding: 20px; flex: 1; padding-bottom: 90px; }}
            .tab-content {{ display: none; }}
            .tab-content.active {{ display: block; }}
            .cases-grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 15px; }}
            ...
            .case-img {{ font-size: 45px; margin: 10px 0; }}
    ...
    <body>
        <header>
            ...
            <div class="balance-container">
                <span id="balance">100.00</span> AimCoin (<span id="uc-balance">60</span> UC)
            </div>
        </header>

        <div class="container">
            <div id="cases-tab" class="tab-content active">
                <h3 style="margin-bottom: 15px;">PUBG Кейсы (20 ta)</h3>
            ...
            <div id="wallet-tab" class="tab-content">
                <div class="panel">
                    <h3 style="margin-bottom: 15px;">Balansni to'ldirish (60 UC = 14,000 so'm)</h3>
                    <div class="form-group">
                        <label>UC miqdori (Minimal 60 UC):</label>
                        <input type="number" id="uc-topup" value="60" oninput="calcSum()">
                    </div>
                    <p style="color: #ffcc00; margin-bottom: 15px; font-size: 14px;">To'lov summasi: <span id="sum-calc">14000</span> so'm</p>
                    ...
                    <button class="btn-submit" onclick="createPayment()">To'lov qildim (Webhook)</button>
            ...
            <div id="promo-tab" class="tab-content">
                <div class="panel">
                    <h3 style="margin-bottom: 15px;">Promokod aktivatsiya</h3>
                    <div class="form-group">
            ...
            <div id="withdraw-tab" class="tab-content">
                <div class="panel">
                    <h3 style="margin-bottom: 15px;">UC Yechib olish</h3>
                    <div class="form-group">
                        <label>Aniq PUBG ID yozing:</label>
                        <input type="text" id="pubg-id-input" placeholder="Masalan: 5123456789">
                    </div>
                    <div class="form-group">
                        <label>Yechib olinadigan UC (Minimal 60 UC to'ldirgan bo'lishi shart):</label>
                        <input type="number" id="uc-withdraw-amount" value="60">
                    </div>
                    <button class="btn-submit" onclick="requestUCWithdraw()">UC Yechish so'rovi</button>
        ...
        <nav class="bottom-nav">
            <button class="nav-item active" onclick="switchTab('cases', this)"><span class="icon">📦</span> Кейсы</button>
            <button class="nav-item" onclick="switchTab('wallet', this)"><span class="icon">💳</span> To'ldirish</button>
            <button class="nav-item" onclick="switchTab('promo', this)"><span class="icon">🎁</span> Promo</button>
            <button class="nav-item" onclick="switchTab('withdraw', this)"><span class="icon">💸</span> UC Yechish</button>
        </nav>
        ...
        <script>
            ...
            async function startRoulette(caseId, priceAim, caseName) {
                if(balanceAim < priceAim) { alert("AimCoin yetarli emas! Balansni to'ldiring."); return; }
                balanceAim -= priceAim;
                updateUI();
...
async def topup_webhook(uc: float = Form(...), user_id: int = Form(1)):
    ...
    conn.commit()
    conn.close()
    return {"success": True, "new_aim": new_aim, "msg": f"To'lov tasdiqlandi! Hisobingizga {uc} UC avtomatik qo'shildi."}
...
async def activate_promo(code: str = Form(...), user_id: int = Form(1)):
    ...
    conn.commit()
    conn.close()
    return {"success": True, "reward_aim": reward_aim, "msg": f"Promokod muvaffaqiyatli ishlatildi! +{reward} UC"}

@app.post("/withdraw_uc")
async def withdraw_uc(pubg_id: str = Form(...), uc: float = Form(...), user_id: int = Form(1)):
    if not pubg_id:
        return {"success": False, "msg": "Iltimos, aniq PUBG ID kiriting!"}
    ...
    if total_donated < 60:
        conn.close()
        return {"success": False, "msg": "UC yechish uchun kamida 60 UC to'ldiring!"}
    
    aim_required = (uc / 60) * 100
    if aimcoin < aim_required:
        conn.close()
        return {"success": False, "msg": "Hisobingizda yetarli mablag' yo'q!"}
    ...
    conn.commit()
    conn.close()
    return {"success": True, "msg": "UC yechish so'rovi adminga yuborildi! Tez orada PUBG ID ga tashlab beriladi."}
